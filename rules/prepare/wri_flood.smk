...
rule clip_wri_flood:
    ...
    output:
        rwi_resampled = "data/inputs/analysis/countries/{ISO3}/{ISO3}_wri-flood_RP{RP}.tif"
    script:
        "./rwi_prep.py"
...
rule clip_wri_coastal_flood:
    ...
    input:
        pop_file="data/inputs/analysis/countries/{ISO3}/{ISO3}_ghs-pop.tif",
        rwi_file=lambda wc: (
            f"data/inputs/flood/WRI_coastal/inuncoast_historical_nosub_hist_rp{'%05d' % int(wc.RP)}.tif"
        ),
        boundary_file="data/inputs/boundaries/{ISO3}/geobounds_{ISO3}.geojson",
    wildcard_constraints:
        RP="2|5|10|25|50|100|250|500|1000"
