"""
Given relative risk maps for Deltares coastal flood return periods - calculate the average annual relative risk
...
def main(RP2_path, RP_5_path, RP10_path, RP25_path, RP50_path, RP100_path, RP250_path, flopros_path, aar_output_path, vuln_dataset):
    logging.basicConfig(format="%(asctime)s %(process)d %(filename)s %(message)s", level=logging.INFO)

    logging.info(f"Calculating Deltares (coastal) average annual relative risk using {vuln_dataset} vulnerability curve.")

    logging.info("Reading raster data.")
    raster_paths = [RP250_path, RP100_path, RP50_path, RP25_path, RP10_path, RP5_path, RP2_path]
...
if __name__ == "__main__":
    ...
    try:
        ...
        flopros__path: str = snakemake.input["flopros"]
        ...
        try:
            ...
            flopros__path: str = sys.argv[8]
            aar_output_path: str = sys.argv[9]
            vuln_dataset: str = sys.argv[10]
        except IndexError:
            print("""\
Example usage: python rules/analyze/deltares_coastal_average_annual_risk_protected.py \\
    data/results/flood_risk/countries/CRI/CRI_deltares-coastal-flood-risk_RP2_V-JRC.tif \\
    data/results/flood_risk/countries/CRI/CRI_deltares-coastal-flood-risk_RP5_V-JRC.tif \\  
    data/results/flood_risk/countries/CRI/CRI_deltares-coastal-flood-risk_RP10_V-JRC.tif \\
    data/results/flood_risk/countries/CRI/CRI_deltares-coastal-flood-risk_RP25_V-JRC.tif \\
    data/results/flood_risk/countries/CRI/CRI_deltares-coastal-flood-risk_RP50_V-JRC.tif \\
    data/results/flood_risk/countries/CRI/CRI_deltares-coastal-flood-risk_RP100_V-JRC.tif \\
    data/results/flood_risk/countries/CRI/CRI_deltares-coastal-flood-risk_RP250_V-JRC.tif \\
    data/inputs/analysis/countries/CRI/CRI_flopros.tif \\
    data/results/flood_risk/countries/CRI/CRI_deltares-coastal-flood-risk_protected_AAR_V-JRC.tif \\
    JRC
""")
        raise ValueError("Must be run via snakemake or with all parameters suppplied.")

    main(RP2_path, RP5_path, RP10_path, RP25_path, RP50_path, RP100_path, RP250_path, flopros__path, aar_output_path, vuln_dataset)